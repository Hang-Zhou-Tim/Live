function addCookie(name, value, days, path) {
    /**Add config for cookie**/
    var name = escape(name);
    var value = escape(value);
    var expires = new Date();
    expires.setTime(expires.getTime() + days * 3600000 * 24);
    //path=/，represent cookie can be used by entire Project
    path = path == "" ? "" : ";path=" + path;
    //GMT to UTC
...
function deleteCookie(name, path) {
    /**Delete cookies parameter based on name**/
    var name = escape(name);
...
function httpPost(url, params) {
    let result = axiosReq.post(url,params).then(function(response){
        ...
        //Define a error message object
        var errorObj = new Object();
        errorObj.code=500;
        errorObj.msg = 'Dear User, The system is failed to send request.';
        return errorObj;
    })
    return result;
}
